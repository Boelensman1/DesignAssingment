...
$(function() {
    ...
    setTimeout(function() {
        ...
        get_target_temp().done(function(result) {
            ...
            set_time(200);
            myApp.hidePreloader();
            if (localStorage.firststartup != "false")
            {
                localStorage.setItem("firststartup", "false");
                myApp.popover('.popover-help', '.button-help')
            }
        });
    }, 2000);
...
$('#set_temps').click(function() {
    ...
    else
    {
        if (manual_temp == true)
        {
            myApp.alert('Setting day/night temperatures while a manual temperature is set. The changes won\'t take effect until after the manual temperature period has ended.', 'Warning', function() {
                show_set_temps();
            });
        }
        else
