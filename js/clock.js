...
function klok_load(dag)
{
    ...
    setTimeout(function() {
        ...
        if (device.iphone) {
            $('#rotationSliderContainer').on({'click': delete_slider});
        }
        else
        {
            $('#rotationSliderContainer').on({'mousedown': delete_slider});
        }
        ...
        get_program().done(function(result) {
            ...
            get_target_temp().done(function(result) {
                ...
                myApp.addNotification({
                    title: 'Weekprogram',
                    message: 'Succesfully set new weekprogram for ' + dagen.join(', '),
                    hold: 3000,
                    closeIcon: false
                });
            });
...
function touchstart_func(e)
{
    ...
    if (timeout == 0)
    {
        ...
        if (move == true)
        {
            ...
            $(this).find('.rotationSlider_time').html(to_time(newdegree));
        ...
        else
        {
            ...
            $(this).removeClass('touchslider_new');
...
function rotate_pie(degrees)
{
    ...
    $(function() {
        if (sliders > 1 || (sliders == 1 && degrees[0] !== 0 && degrees[0] !== 360))
        {
            //degrees.shift();
            $('#pie').html('');
...
function add_slider(degree, neww)
{
    if (degree == 360)
    {
        degree = 0;
    ...
    if (neww == false)
    {
        $container.append('<div id="rotationSlider_' + (new_sliderid) + '" class="rotationSlider"><div class="rotationSlider_time">' + to_time(degree) + '</div></div>');
    }
    else
    {
        $container.append('<div id="rotationSlider_' + (new_sliderid) + '" class="rotationSlider touchslider_new"><div class="rotationSlider_time">NEW</div></div>');
    }
    var deg = degree;
    ...
    if (device.iphone) {
        ...
        $('.rotationSlider').on({'mousedown': touchstart_func});
        $('.rotationSlider').on({'mouseup': touchend_func});
    }
    if (sliders == 10)
    {
    ...
    if (sliders < 10)
    {
        ...
        if (sliders > 0)
        {
            ...
            if (sliders < 9)
            {
                ...
                add_slider(new_degree1, true);
                add_slider(new_degree2, true);
            }
            else
            {
